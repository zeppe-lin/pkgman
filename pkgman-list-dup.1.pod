...
Print list of duplicate packages in the packages sources directories. It
looks in all directories specified in F</etc/pkgman.conf> as C<pkgsrcdir>.
...
Prefer higher installed versions over lower ones in the packages sources
directories.
...
Use C<\n> and C<\t> to format your output. No additional format specifier is
supported.
