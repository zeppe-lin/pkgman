...
C<pkgsrcdir> options is important as if a package is in multiple directories,
L<pkgman(1)|https://github.com/zeppe-lin/zeppe-lin/blob/master/pkgman/pkgman.1.pod>
will use the one found first (directories listed first have precedence).

If you want to restrict the packages used from one C<pkgsrcdir>, you can
append a comma-separated list of packages to be used after the path, using
a colon C<:> character to separate the two components.
...
If set to I<yes>,
L<pkgman(1)|https://github.com/zeppe-lin/zeppe-lin/blob/master/pkgman/pkgman.1.pod>
will parse version strings and prefer the higher one, even if the one found
in the packages sources directories is lower. Influences C<diff>, C<sysup>
operations.
...
If set to I<yes>,
L<pkgman(1)|https://github.com/zeppe-lin/zeppe-lin/blob/master/pkgman/pkgman.1.pod>
will interpret C<expr> and C<filter> patterns in list, printf, dsearch and
fsearch operations as regular expression.
...
See
L<pkgman-readme(1)|https://github.com/zeppe-lin/zeppe-lin/blob/master/pkgman/pkgman-readme.1.pod>
...
The string that contains a path to the log file, where all occurrences of next
characters will be replaced by corresponding items:
...
Use an alternate command instead of L<sh(1)> for the command which is used
