//! \file      prt.h
//! \brief     Prt main commands definition
...
//! \class  Prt
//! \brief  Prt's main class, controlling all the commands
class Prt : public SignalHandler
{
public:
  //! \brief   Create a Prt object
  //!
  //! \param   parser  the argument parser to be used
  Prt( const ArgParser* parser );

  //! Destroy a Prt object
  ~Prt();
  ...
  //! Print list of orphans packages
  void listOrphans();
  ...
  //! Print a listing of the port's directory
  void ls();

  //! Print the port's file
  void cat();

  //! Edit the port's file
  void edit();
