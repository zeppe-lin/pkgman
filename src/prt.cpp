//! \file       prt.cpp
//! \brief      Prt main commands implementation
...
void Prt::printDiff()
{
  ...
  size_t different_packages = 0, new_packages = 0;
  ...
  if ( m_greaterVersionComp.size() )
  {
    ...
    for ( const auto& name: m_greaterVersionComp )
    {
      m_pkgDB->isInstalled( name ) ? ++different_packages
                                   : ++new_packages;
  ...
  if ( ! different_packages && ! new_packages )
  {
  ...
  if ( different_packages )
  {
    ...
    cout.width( 3 );
    cout << different_packages;
    cout << " " <<
      ( different_packages > 1 ? "updates" : "update" ) << endl;
  }
  ...
  if ( new_packages )
  {
    ...
    cout << new_packages;
    cout << " " <<
      ( new_packages  > 1 ? "installs" : "install" ) << endl;
  }
}

void Prt::printMissingDep()
{
  map< pkgname_t, pkgver_t > packages;
...
void Prt::install( Transaction::Transaction_t transaction_t )
{
...
void Prt::edit()
{
  ...
  const auto& pkg = m_repo->getPackage( name );
  if ( ! pkg )
    return errx( "package '" + name + "' not found" );
...
Prt::handleSignal( int __attribute__((unused)) signal )
{
...
void Prt::initRepo( bool listDuplicate )
{
  if ( m_repo )
    return;
...
void Prt::expandWildcardsPkgDB( const list< char* >&         in,
...
void Prt::expandWildcardsRepo( const list< char* >&  in,
                               list< pkgname_t >&    target )
{
  for ( const auto& pattern: in )
  {
...
void Prt::listDepSorted( map< string, string >& packages )
{
  while ( packages.size() )
  {
...
bool Prt::printFile( const string& filepath )
{
...
COMP_RESULT Prt::compareVersions( const string& v1, const string& v2 )
{
  if ( v1 == v2 )
    return EQUAL;
...
void Prt::diffCalc( const map< pkgname_t, pkgver_t >& packages )
{
  for ( const auto& [ name, version ]: packages )
  {
    const auto& pkg = m_repo->getPackage( name );
...
void Prt::printFormattedDiffLine( const string&  name,
...
void Prt::printDepList( const Package* pkg, int level )
{
  if ( pkg->dependencies().empty() )
    return;
...
void Prt::printDepTree( const Package* pkg, int level )
{
  if ( pkg->dependencies().empty() )
    return;
...
void Prt::printRevDepList( const pkgname_t& arg, int level )
{
  set< string > revdep;
...
void Prt::printRevDepTree( const pkgname_t& arg, int level )
{
...
void Prt::getRemovableDeps( const string&    name,
                            list< string >&  removable )
{
  const auto& pkg = m_repo->getPackage( name );
...
bool Prt::footprintGrep( const string&    filename,
...
void
Prt::executeTransaction( Transaction&                transaction,
...
void Prt::evaluateResult( const Transaction&  transaction,
                          bool                interrupted )
{
  int errors = 0;
