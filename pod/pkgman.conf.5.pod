...
This manual page describes the format of the F<pkgman.conf> file, that
contains information for L<pkgman(1)>.

Every line can contain one configuration option.  The format of the
lines is C<key value>.  Any line B<beginning> with a hash (B<#>) is
...
directories, L<pkgman(1)> will use the one found first (directories
listed first have precedence).
...
If set to I<yes>, L<pkgman(1)> will parse version strings and prefer
the higher one, even if the one found in the packages sources
directories is lower.  Influences I<diff> and I<sysup> operations.

Default: I<no>.

=item B<useregex> (I<yes>|I<no>)

If set to I<yes>, L<pkgman(1)> will interpret C<expr> and C<filter>
patterns in I<list>, I<printf>, I<dsearch> and I<fsearch> operations
as regular expression in L<regex(3)> format.
...
L<pkgman(1)> will notify the user if installed package has a README
file.
...
See L<pkgman-readme(1)> for information how to read those README
files.
...
Write the log for sysup, install, update and remove operations.
...
Use an alternate I<command> instead of L<pkgmk(8)> for the command
...
Use an alternate I<command> instead of L<pkgadd(8)> for the command
...
Use an alternate I<command> instead of L<pkgrm(8)> for the command
...
Use an alternate command instead of L<sh(1p)> for the command which is
