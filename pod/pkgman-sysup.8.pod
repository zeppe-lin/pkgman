...
Also see the L<pkgman-lock(8)> and L<pkgman-unlock(8)> commands to
keep the currently installed versions, even if there are changes in
the packages sources directories.
