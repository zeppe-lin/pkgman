...
B<pkgman> [I<OPTION>]... [I<COMMAND>] [I<ARGS>]
...
Ignore default (F</etc/pkgman.conf>) configuration file.
...
processing.  Any arguments after the B<--> are treated as I<COMMAND>
and/or I<COMMAND>'s arguments.  It helps to specify I<COMMAND>
arguments that starts with B<-> and B<pkgman> won't confuse it with an
option.  For example:
...
search the packages sources directories for package name B<and>
description
